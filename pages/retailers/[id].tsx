...
export default function Retailer() {
  ...
  async function getTotalPayments(userId) {
    try {
      setIsLoading(true);
      const { data } = await supabase.from('payments').select('*').eq('retailer_id', userId);
      setTotalPayments(findTotalSum(data.map((d) => d.amount)));
    } catch (error) {
      toast.error(
        'Ocurrió un error al consultar la información del vendedor, recarga para intentarlo de nuevo',
      );
    } finally {
      setIsLoading(false);
    }
  ...
  async function getRetailerInfo(userId) {
    try {
      setIsLoading(true);
      const { data } = await supabase.from('retailers').select('*').eq('id', userId).single();
      setRetailer(data);
    } catch (error) {
      toast.error(
        'Ocurrió un error al consultar la información del vendedor, recarga para intentarlo de nuevo',
  ...
  async function getRetailerSales(userId) {
    try {
      setIsLoading(true);
      const { data } = await supabase
        ...
        .eq('retailer', userId);
      setSales(data);
      setTotalSalesNoPandora(
        findTotalSum(data.filter((d) => !d.products.pandora).map((d) => d.sale_price)),
      );
      setTotalSalesPandora(
        findTotalSum(data.filter((d) => d.products.pandora).map((d) => d.sale_price)),
      );
    } catch (error) {
      toast.error(
  ...
  const handleBarClick = (e) => {
    ...
    setYearPreSelected(moment().year());
    setMonthPreSelected(index + 1);
  };
    ...
    <>
      ...
      <div className="flex gap-6 sm:gap-2 flex-col sm:flex-row">
        <div className="flex-1 py-12 flex rounded-lg flex-col justify-center items-center bg-white">
          ...
          <span className="uppercase mt-2 text-xs text-zinc-400 tracking-wider">
            Total de ventas
          </span>
        </div>
      ...
      <div className="flex flex-col w-lg h-80 px-10 py-6 bg-white mt-10 rounded-xl shadow-xl shadow-zinc-200/10 relative">
        ...
        <div className="flex-1">
          {sales.length > 0 && (
            <Bar ref={chartRef} options={options} data={chart} onClick={handleBarClick} />
          )}
        </div>
      </div>
      <div className="flex flex-col w-lg py-6 px-10 bg-white mt-10 rounded-xl shadow-xl shadow-zinc-200/10 relative">
        {retailer && (
          <SalesTable
            setIsLoading={setIsLoading}
            tableTitle={`Ventas de ${retailer?.name}`}
            retailer={retailer}
            yearPreSelected={yearPreSelected}
            monthPreselected={monthPreSelected}
          />
        )}
      </div>
    </>

...
export default function Dashboard(props: any) {
  ...
  const [productConsignmentCount, setproductConsignmentCount] = useState(0);
  ...
  async function getProductsCount() {
    const { count } = await supabase
      ...
      .eq('status', AVAILABLE);
    const { count: consignmentsCount } = await supabase
      .from('products')
      .select('id', { count: 'exact' })
      .eq('status', CONSIGNMENT);
    setProductCount(count);
    setproductConsignmentCount(consignmentsCount);
  }
  ...
  return isSignedIn ? (
    <div className="w-full h-full bg-zinc-50 dark:bg-zinc-800 flex">
      ...
      >
        <button
          className="md:hidden absolute right-10 top-10 rounded-full p-4"
        ...
        <div>
          ...
          {userMeta?.roles?.id === 1 && (
            <Link href="/retailers" legacyBehavior>
              <a
                onClick={() => setMenuVisible(false)}
                className={`font-lg my-5 rounded-full py-3 px-6 text-sm flex items-center ease-in-out duration-300 ${
                  current === 'retailers'
          ...
          {userMeta?.roles?.id === 1 && (
            <Link href="/consignments" legacyBehavior>
              <a
                onClick={() => setMenuVisible(false)}
                className={`font-lg my-5 rounded-full py-3 px-6 text-sm flex items-center ease-in-out duration-300 ${
              ...
              >
                ...
                <div
                  className={`flex w-6 h-6 text-xs items-center justify-center rounded-full absolute right-8 top-50 ${
                    current === 'consignments' ? 'text-white' : 'text-zinc-700'
                  }`}
                >
                  {productConsignmentCount}
                </div>
              </a>
            </Link>
          )}
